...
function render() {
  ...
  renderFooter(shell, sectionEls);
  app.appendChild(shell);
}

function renderFooter(shell, sectionEls) {
  const cfg = window.PORTFOLIO_CONFIG || {};
  ...
  // ── Top grid ──────────────────────────────────────────────
  const grid = createEl("div", "footer-grid");

  // Brand column
  const brandCol = createEl("div", "footer-col footer-col--brand");
  const logoImg = document.createElement("img");
  logoImg.src = "resources/Certificates/Logo.svg";
  logoImg.alt = "Logo";
  logoImg.className = "footer-logo";
  const tagline = createEl("p", "footer-tagline");
  tagline.innerHTML = `<strong>${cfg.headlinePrimary || "Full Stack Developer"}</strong> crafting efficient digital experiences tailored to <strong>real business needs.</strong> Based in ${cfg.locationLabel || "Philippines"}.`;
  brandCol.appendChild(logoImg);
  brandCol.appendChild(tagline);

  // Quick Links column
  const linksCol = createEl("div", "footer-col");
  const linksHeading = createEl("h3", "footer-heading", "Quick Links");
  const linksList = createEl("ul", "footer-links");
  ["home", "about", "skills", "projects", "publications", "certificates"].forEach((id) => {
    const li = document.createElement("li");
    const btn = createEl("button", "footer-link-btn", id.toUpperCase());
    btn.addEventListener("click", () => {
  ...
  linksCol.appendChild(linksHeading);
  linksCol.appendChild(linksList);

  // Connect column
  const connectCol = createEl("div", "footer-col");
  const connectHeading = createEl("h3", "footer-heading", "Connect");
  const socialList = createEl("ul", "footer-socials");
  const socialIcons = {
    ...
    email: `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="4" width="20" height="16" rx="2"/><polyline points="2,4 12,13 22,4"/></svg>`,
  };
  (cfg.socials || []).forEach((s) => {
    const li = document.createElement("li");
    ...
    const iconSpan = createEl("span", "footer-social-icon");
    iconSpan.innerHTML = socialIcons[s.type] || "";
    const label = createEl("span", "footer-social-label", (s.type || "").toUpperCase());
    a.appendChild(iconSpan);
    a.appendChild(label);
    li.appendChild(a);
    socialList.appendChild(li);
  });
  connectCol.appendChild(connectHeading);
  connectCol.appendChild(socialList);

  grid.appendChild(brandCol);
  grid.appendChild(linksCol);
  grid.appendChild(connectCol);

  // ── Bottom bar ────────────────────────────────────────────
  const bottom = createEl("div", "footer-bottom");
  const copy = createEl(
    "span", "footer-copy",
    `© ${year} ${cfg.nameFirst || ""} ${cfg.nameLast || ""} • All rights reserved`
  );
  const backTop = document.createElement("button");
  backTop.className = "footer-back-top";
  backTop.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="18,15 12,9 6,15"/></svg><span>BACK TO TOP</span>`;
  backTop.addEventListener("click", () => window.scrollTo({ top: 0, behavior: "smooth" }));
  bottom.appendChild(copy);
  bottom.appendChild(backTop);
